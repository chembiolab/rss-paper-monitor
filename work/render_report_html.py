...
for i,p in enumerate(r['items'],1):
    ...
    parts += [f'<article><h2>{i}. {esc(p["title"])}</h2>',f'<p class="tags"><b>저널</b> {esc(p["journal"] or "확인 불가")} · <b>분류</b> {esc(", ".join(p["labels"]))}</p>',f'<p><b>저자</b> {esc(p.get("authors") or "저자 정보 없음")}</p>',f'<p><b>출처</b> {esc(p["source"])} · <b>DOI</b> {doi} · <b>PMID</b> {pmid}</p>',f'<p><b>원문 또는 PubMed</b> {link(p["url"])}</p>'</article>']
